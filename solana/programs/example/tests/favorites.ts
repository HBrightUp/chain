...
describe('Favorites', () => {
  ...
  const favoriteNumber = new anchor.BN(100);
  ...
  it('Writes our favorites to the blockchain', async () => {
    ...
    console.log("setFavorites signature:", signature);
    ...
    console.log("Writes our favorites ", dataFromPda);
  ...
  it('Updates the favorites', async () => {
    ...
    console.log(`Updates favorites of transaction signature: ${signature}`);

    const favoritesPdaAndBump = web3.PublicKey.findProgramAddressSync([Buffer.from('favorites'), user.publicKey.toBuffer()], program.programId);
    const favoritesPda = favoritesPdaAndBump[0];
    const dataFromPda = await program.account.favorites.fetch(favoritesPda);
    console.log("Updates our favorites ", dataFromPda);
