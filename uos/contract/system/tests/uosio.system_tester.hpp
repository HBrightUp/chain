...
namespace uosio_system {

class uosio_system_tester : public TESTER {
public:

   void basic_setup() {
      produce_blocks( 2 );

      create_accounts({ N(uosio.token), N(uosio.ram), N(uosio.ramfee), N(uosio.stake),
               N(uosio.bpay), N(uosio.vpay), N(uosio.saving), N(uosio.names), N(uosio.rex) });
      ...
      {
         const auto& accnt = control->db().get<account_object,by_name>( N(uosio.token) );
         abi_def abi;
   ...
   void create_core_token( symbol core_symbol = symbol{CORE_SYM} ) {
      ...
      create_currency( N(uosio.token), config::system_account_name, asset(100000000000000, core_symbol) );
      issue(config::system_account_name, asset(10000000000000, core_symbol) );
      BOOST_REQUIRE_EQUAL( asset(10000000000000, core_symbol), get_balance( "uosio", core_symbol ) );
   }
   ...
   void remaining_setup() {
      ...
      BOOST_REQUIRE_EQUAL( core_sym::from_string("1000000000.0000"), get_balance("uosio")  + get_balance("uosio.ramfee") + get_balance("uosio.stake") + get_balance("uosio.ram") );
   }
   ...
   uosio_system_tester( setup_level l = setup_level::full ) {
   ...
   uosio_system_tester(Lambda setup) {
      setup(*this);
   ...
   action_result buyram( const account_name& payer, account_name receiver, const asset& uosin ) {
      return push_action( payer, N(buyram), mvo()( "payer",payer)("receiver",receiver)("quant",uosin) );
   }
   ...
   fc::variant get_last_loan(bool cpu) {
      ...
      namespace chain = uosio::chain;
      auto table = cpu ? N(cpuloan) : N(netloan);
      const auto* t_id = db.find<uosio::chain::table_id_object, chain::by_code_scope_table>( boost::make_tuple( config::system_account_name, config::system_account_name, table ) );
      if ( !t_id ) {
         return fc::variant();
      }
   ...
   fc::variant get_rex_pool() const {
      ...
      const auto& db = control->db();
      namespace chain = uosio::chain;
      const auto* t_id = db.find<uosio::chain::table_id_object, chain::by_code_scope_table>( boost::make_tuple( config::system_account_name, config::system_account_name, N(rexpool) ) );
      if ( !t_id ) {
         return fc::variant();
      }
   ...
   asset get_balance( const account_name& act, symbol balance_symbol = symbol{CORE_SYM} ) {
      vector<char> data = get_row_by_account( N(uosio.token), act, N(accounts), balance_symbol.to_symbol_code().value );
   ...
   fc::variant get_stats( const string& symbolname ) {
      auto symb = uosio::chain::symbol::from_string(symbolname);
      auto symbol_code = symb.to_symbol_code().value;
      vector<char> data = get_row_by_account( N(uosio.token), symbol_code, N(stat), symbol_code );
   ...
   fc::variant get_global_state() {
      ...
      return data.empty() ? fc::variant() : abi_ser.binary_to_variant( "uosio_global_state", data, abi_serializer_max_time );
   }
   ...
   fc::variant get_global_state2() {
      ...
      return data.empty() ? fc::variant() : abi_ser.binary_to_variant( "uosio_global_state2", data, abi_serializer_max_time );
   }
   ...
   fc::variant get_global_state3() {
      ...
      return data.empty() ? fc::variant() : abi_ser.binary_to_variant( "uosio_global_state3", data, abi_serializer_max_time );
   }
   ...
   abi_serializer initialize_multisig() {
      ...
      {
         create_account_with_resources( N(uosio.msig), config::system_account_name );
         BOOST_REQUIRE_EQUAL( success(), buyram( "uosio", "uosio.msig", core_sym::from_string("5000.0000") ) );
         produce_block();
   ...
   vector<name> active_and_vote_producers() {
      ...
      transfer( "uosio", "alice1111111", core_sym::from_string("650000000.0000"), "uosio" );
      ...
      auto trace_auth = TESTER::push_action(config::system_account_name, updateauth::get_name(), config::system_account_name, mvo()
                                            ...
                                            ("auth",  authority(1, {key_weight{get_public_key( config::system_account_name, "active" ), 1}}, {
                                                  permission_level_weight{{config::system_account_name, config::uosio_code_name}, 1},
...
inline uint64_t M( const string& uos_str ) {
   return core_sym::from_string( uos_str ).get_amount();
}
