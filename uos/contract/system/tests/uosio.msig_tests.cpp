...
class uosio_msig_tester : public tester {
public:
   uosio_msig_tester() {
      create_accounts( { N(uosio.msig), N(uosio.stake), N(uosio.ram), N(uosio.ramfee), N(alice), N(bob), N(carol) } );
      produce_block();
      ...
      set_code( N(uosio.msig), contracts::msig_wasm() );
      set_abi( N(uosio.msig), contracts::msig_abi().data() );

      produce_blocks();
      const auto& accnt = control->db().get<account_object,by_name>( N(uosio.msig) );
      abi_def abi;
   ...
   transaction_trace_ptr create_account_with_resources( account_name a, account_name creator, asset ramfunds, bool multisig,
      ...
      trx.actions.emplace_back( get_action( N(uosio), N(buyram), vector<permission_level>{{creator,config::active_name}},
                                            mvo()
                                            ("payer", creator)
                                            ("receiver", a)
                                            ("quant", ramfunds) )
      ...
      trx.actions.emplace_back( get_action( N(uosio), N(delegatebw), vector<permission_level>{{creator,config::active_name}},
                                            mvo()
                                            ("from", creator)
                                            ("receiver", a)
   ...
   void issue( name to, const asset& amount, name manager = config::system_account_name ) {
      base_tester::push_action( N(uosio.token), N(issue), manager, mutable_variant_object()
                                ("to",      to )
                                ("quantity", amount )
                                ("memo", "")
                                );
   ...
   void transfer( name from, name to, const string& amount, name manager = config::system_account_name ) {
      base_tester::push_action( N(uosio.token), N(transfer), manager, mutable_variant_object()
                                ("from",    from)
                                ("to",      to )
   ...
   asset get_balance( const account_name& act ) {
      ...
      const auto* tbl = db.find<table_id_object, by_code_scope_table>(boost::make_tuple(N(uosio.token), act, N(accounts)));
      share_type result = 0;
   ...
   transaction_trace_ptr push_action( const account_name& signer, const action_name& name, const variant_object& data, bool auth = true ) {
      ...
      auto trace = base_tester::push_action( N(uosio.msig), name, accounts, data );
      produce_block();
...
transaction uosio_msig_tester::reqauth( account_name from, const vector<permission_level>& auths, const fc::microseconds& max_serialization_time ) {
   fc::variants v;
...
BOOST_AUTO_TEST_SUITE(uosio_msig_tests)

BOOST_FIXTURE_TEST_CASE( propose_approve_execute, uosio_msig_tester ) try {
   ...
   BOOST_REQUIRE_EXCEPTION( push_action( N(alice), N(exec), mvo()
                            ...
                            uosio_assert_message_is("transaction authorization failed")
   );
...
BOOST_FIXTURE_TEST_CASE( propose_approve_unapprove, uosio_msig_tester ) try {
   auto trx = reqauth("alice", {permission_level{N(alice), config::active_name}}, abi_serializer_max_time );
...
BOOST_FIXTURE_TEST_CASE( propose_approve_by_two, uosio_msig_tester ) try {
...
BOOST_FIXTURE_TEST_CASE( propose_with_wrong_requested_auth, uosio_msig_tester ) try {
   auto trx = reqauth("alice", vector<permission_level>{ { N(alice), config::active_name },  { N(bob), config::active_name } }, abi_serializer_max_time );
...
BOOST_FIXTURE_TEST_CASE( big_transaction, uosio_msig_tester ) try {
...
BOOST_FIXTURE_TEST_CASE( update_system_contract_all_approve, uosio_msig_tester ) try {

   // required to set up the link between (uosio active) and (uosio.prods active)
   //
   //                  uosio active
   //                       |
   //             uosio.prods active (2/3 threshold)
   //             /         |        \             <--- implicitly updated in onblock action
   // alice active     bob active   carol active

   set_authority(N(uosio), "active", authority(1,
      vector<key_weight>{{get_private_key("uosio", "active").get_public_key(), 1}},
      vector<permission_level_weight>{{{N(uosio.prods), config::active_name}, 1}}), "owner",
      { { N(uosio), "active" } }, { get_private_key( N(uosio), "active" ) });

   set_producers( {N(alice),N(bob),N(carol)} );
   produce_blocks(50);

   create_accounts( { N(uosio.token), N(uosio.rex) } );
   set_code( N(uosio.token), contracts::token_wasm() );
   set_abi( N(uosio.token), contracts::token_abi().data() );

   create_currency( N(uosio.token), config::system_account_name, core_sym::from_string("10000000000.0000") );
   ...
   BOOST_REQUIRE_EQUAL( core_sym::from_string("1000000000.0000"),
                        get_balance("uosio") + get_balance("uosio.ramfee") + get_balance("uosio.stake") + get_balance("uosio.ram") );
   ...
   produce_blocks();
   create_account_with_resources( N(alice1111111), N(uosio), core_sym::from_string("1.0000"), false );
   create_account_with_resources( N(bob111111111), N(uosio), core_sym::from_string("0.4500"), false );
   create_account_with_resources( N(carol1111111), N(uosio), core_sym::from_string("1.0000"), false );
   ...
   vector<permission_level> action_perm = {{N(uosio), config::active_name}};
   ...
   // execute by alice to replace the uosio system contract
   transaction_trace_ptr trace;
   control->applied_transaction.connect([&]( const transaction_trace_ptr& t) { if (t->scheduled) { trace = t; } } );
   ...
   BOOST_REQUIRE_EXCEPTION( create_account_with_resources( N(alice1111112), N(uosio), core_sym::from_string("1.0000"), false ),
                            uosio_assert_message_exception, uosio_assert_message_is("Unknown Test")

   );
} FC_LOG_AND_RETHROW()

BOOST_FIXTURE_TEST_CASE( update_system_contract_major_approve, uosio_msig_tester ) try {

   // set up the link between (uosio active) and (uosio.prods active)
   set_authority(N(uosio), "active", authority(1,
      vector<key_weight>{{get_private_key("uosio", "active").get_public_key(), 1}},
      vector<permission_level_weight>{{{N(uosio.prods), config::active_name}, 1}}), "owner",
      { { N(uosio), "active" } }, { get_private_key( N(uosio), "active" ) });
   ...
   create_currency( N(uosio.token), config::system_account_name, core_sym::from_string("10000000000.0000") );
   issue(config::system_account_name, core_sym::from_string("1000000000.0000"));
   BOOST_REQUIRE_EQUAL( core_sym::from_string("1000000000.0000"), get_balance( "uosio" ) );
   ...
   BOOST_REQUIRE_EXCEPTION(
      ...
      uosio_assert_message_exception, uosio_assert_message_is("transaction authorization failed")
   );
...
BOOST_FIXTURE_TEST_CASE( propose_approve_invalidate, uosio_msig_tester ) try {
   auto trx = reqauth("alice", {permission_level{N(alice), config::active_name}}, abi_serializer_max_time );
...
BOOST_FIXTURE_TEST_CASE( propose_invalidate_approve, uosio_msig_tester ) try {
   auto trx = reqauth("alice", {permission_level{N(alice), config::active_name}}, abi_serializer_max_time );
...
BOOST_FIXTURE_TEST_CASE( approve_execute_old, uosio_msig_tester ) try {
   set_code( N(uosio.msig), contracts::util::msig_wasm_old() );
   set_abi( N(uosio.msig), contracts::util::msig_abi_old().data() );
   produce_blocks();

   //propose with old version of uosio.msig
   auto trx = reqauth("alice", {permission_level{N(alice), config::active_name}}, abi_serializer_max_time );
   push_action( N(alice), N(propose), mvo()
                  ("proposer",      "alice")
                  ("proposal_name", "first")
...
BOOST_FIXTURE_TEST_CASE( approve_unapprove_old, uosio_msig_tester ) try {
   set_code( N(uosio.msig), contracts::util::msig_wasm_old() );
   set_abi( N(uosio.msig), contracts::util::msig_abi_old().data() );
   produce_blocks();
...
BOOST_FIXTURE_TEST_CASE( approve_by_two_old, uosio_msig_tester ) try {
   set_code( N(uosio.msig), contracts::util::msig_wasm_old() );
   set_abi( N(uosio.msig), contracts::util::msig_abi_old().data() );
   produce_blocks();
...
BOOST_FIXTURE_TEST_CASE( approve_with_hash, uosio_msig_tester ) try {
   auto trx = reqauth("alice", {permission_level{N(alice), config::active_name}}, abi_serializer_max_time );
   ...
   BOOST_REQUIRE_EXCEPTION( push_action( N(alice), N(approve), mvo()
                            ...
                            uosio::chain::crypto_api_exception,
...
BOOST_FIXTURE_TEST_CASE( switch_proposal_and_fail_approve_with_hash, uosio_msig_tester ) try {
