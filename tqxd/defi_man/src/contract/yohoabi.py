...
class Yoho(object):
    ...
    def addWhiteList(self,privkey, to_address):
        pk = int(privkey, 16)
        sendfrom = web3.eth.Account.privateKeyToAccount(pk)
        privateKey = sendfrom._key_obj
        publicKey = privateKey.public_key
        ...
        gasprice = self.web3.eth.gasPrice
        nonce = self.web3.eth.getTransactionCount(address, "pending")
        unsigned_tx = self.contract.functions.addWhiteList(chktoaddress).buildTransaction({'gas':600000,
                                                        'gasPrice':gasprice, 'from': address, 'nonce':nonce})
        signed_tx =  sendfrom.signTransaction(unsigned_tx)
        txid = self.web3.eth.sendRawTransaction(signed_tx.rawTransaction)
        return txid.hex()
    ...
    def resetLimiteTransfer(self, privkey, limit_transfer):
        pk = int(privkey, 16)
        sendfrom = web3.eth.Account.privateKeyToAccount(pk)
        privateKey = sendfrom._key_obj
        publicKey = privateKey.public_key
        ...
        gasprice = self.web3.eth.gasPrice
        nonce = self.web3.eth.getTransactionCount(address, "pending")
        unsigned_tx = self.contract.functions.resetLimiteTransfer(limit_transfer).buildTransaction({'gas':600000,
                                                        'gasPrice':gasprice, 'from': address, 'nonce':nonce})
        signed_tx =  sendfrom.signTransaction(unsigned_tx)
        txid = self.web3.eth.sendRawTransaction(signed_tx.rawTransaction)
        return txid.hex()

    def setBasisPointsRate(self, privkey, basic_point_rate):
        if basic_point_rate > 10000:
            dataformat.logger.error("rate upper limit")
            return
        ...
        gasprice = self.web3.eth.gasPrice
        nonce = self.web3.eth.getTransactionCount(address, "pending")
        unsigned_tx = self.contract.functions.setBasisPointsRate(basic_point_rate).buildTransaction({'gas':600000,
                                                        'gasPrice':gasprice, 'from': address, 'nonce':nonce})
        signed_tx =  sendfrom.signTransaction(unsigned_tx)
        txid = self.web3.eth.sendRawTransaction(signed_tx.rawTransaction)
        return txid.hex()
    ...
    def mint(self, privkey, amount):
        ...
        txid = self.web3.eth.sendRawTransaction(signed_tx.rawTransaction)
        return txid.hex()
    def transfer(self, privkey, to, amount):
        pk = int(privkey, 16)
        sendfrom = web3.eth.Account.privateKeyToAccount(pk)
        privateKey = sendfrom._key_obj
        publicKey = privateKey.public_key
        address = publicKey.to_checksum_address()
        ckto = web3.Web3.toChecksumAddress(to)
        
        gasprice = self.web3.eth.gasPrice
        nonce = self.web3.eth.getTransactionCount(address, "pending")
        unsigned_tx = self.contract.functions.transfer(ckto,amount).buildTransaction({'gas':600000,
                                                        'gasPrice':gasprice, 'from': address, 'nonce':nonce})
        signed_tx =  sendfrom.signTransaction(unsigned_tx)
        txid = self.web3.eth.sendRawTransaction(signed_tx.rawTransaction)
        return txid.hex()
...
class Level(object):       
    ...
    def checkAdmin(self, address):
        ck_address = web3.Web3.toChecksumAddress(address)
        admin =  self.contract.caller().owner()
        print(admin)
        print(ck_address)
        if admin == ck_address: 
            return True
    ...
    def importRelation(self, privkey,upper, lower):
        ckupper = web3.Web3.toChecksumAddress(upper)
        cklower = web3.Web3.toChecksumAddress(lower)
        pk = int(privkey, 16)
        sendfrom = web3.eth.Account.privateKeyToAccount(pk)
        privateKey = sendfrom._key_obj
        publicKey = privateKey.public_key
        ...
        gasprice = self.web3.eth.gasPrice
        nonce = self.web3.eth.getTransactionCount(address, "pending")
        unsigned_tx = self.contract.functions.importRelation(ckupper, cklower).buildTransaction({'gas':6000000,
                                                        'gasPrice':gasprice, 'from': address, 'nonce':nonce})
        signed_tx =  sendfrom.signTransaction(unsigned_tx)
        txid = self.web3.eth.sendRawTransaction(signed_tx.rawTransaction)
        return txid.hex()

    def getUpper(self, lower):
        ck_lower = web3.Web3.toChecksumAddress(lower)
        upper = self.contract.caller().getUpper(ck_lower)
        return upper
...
class StakeRewards(object):       
    ...
    def setLowerLimitAmount(self, privkey, amount):
        pk = int(privkey, 16)
        sendfrom = web3.eth.Account.privateKeyToAccount(pk)
        privateKey = sendfrom._key_obj
        publicKey = privateKey.public_key
        ...
        gasprice = self.web3.eth.gasPrice
        nonce = self.web3.eth.getTransactionCount(address, "pending")
        unsigned_tx = self.contract.functions.setLowerLimitAmount(amount).buildTransaction({'gas':6000000,
                                                        'gasPrice':gasprice, 'from': address, 'nonce':nonce})
        signed_tx =  sendfrom.signTransaction(unsigned_tx)
        txid = self.web3.eth.sendRawTransaction(signed_tx.rawTransaction)
        return txid.hex()

    def updateAdmin(self, privkey, to_address):
        pk = int(privkey, 16)
        sendfrom = web3.eth.Account.privateKeyToAccount(pk)
        privateKey = sendfrom._key_obj
        publicKey = privateKey.public_key
        ...
        gasprice = self.web3.eth.gasPrice
        nonce = self.web3.eth.getTransactionCount(address, "pending")
        unsigned_tx = self.contract.functions.updateAdmin(chktoaddress).buildTransaction({'gas':600000,
                                                        'gasPrice':gasprice, 'from': address, 'nonce':nonce})
        signed_tx =  sendfrom.signTransaction(unsigned_tx)
        txid = self.web3.eth.sendRawTransaction(signed_tx.rawTransaction)
        return txid.hex()

    def setLevelRewardRate(self, privkey, rate):
        if rate > 10000:
            dataformat.logger.error("rate over limit")
            return 
        pk = int(privkey, 16)
        sendfrom = web3.eth.Account.privateKeyToAccount(pk)
        privateKey = sendfrom._key_obj
        ...
        gasprice = self.web3.eth.gasPrice
        nonce = self.web3.eth.getTransactionCount(address, "pending")
        unsigned_tx = self.contract.functions.setLevelRewardRate(rate).buildTransaction({'gas':600000,
                                                        'gasPrice':gasprice, 'from': address, 'nonce':nonce})
        signed_tx =  sendfrom.signTransaction(unsigned_tx)
        txid = self.web3.eth.sendRawTransaction(signed_tx.rawTransaction)
        return txid.hex()

    def setUpperHold(self, privkey, hold):
        pk = int(privkey, 16)
        sendfrom = web3.eth.Account.privateKeyToAccount(pk)
        privateKey = sendfrom._key_obj
        publicKey = privateKey.public_key
        ...
        gasprice = self.web3.eth.gasPrice
        nonce = self.web3.eth.getTransactionCount(address, "pending")
        unsigned_tx = self.contract.functions.setUpperHold(hold).buildTransaction({'gas':600000,
                                                        'gasPrice':gasprice, 'from': address, 'nonce':nonce})
        signed_tx =  sendfrom.signTransaction(unsigned_tx)
        txid = self.web3.eth.sendRawTransaction(signed_tx.rawTransaction)
        return txid.hex()
...
class InviteRewards(object):       
    ...
    def importStakeRelation(self, privkey, lower):
        pk = int(privkey, 16)
        sendfrom = web3.eth.Account.privateKeyToAccount(pk)
        privateKey = sendfrom._key_obj
        publicKey = privateKey.public_key
        ...
        if gasprice > 2000000000:
            gasprice = 2000000000
        nonce = self.web3.eth.getTransactionCount(address, "pending")
        chklower = web3.Web3.toChecksumAddress(lower)
        unsigned_tx = self.contract.functions.importStakeRelation(chklower).buildTransaction({'gas':6000000,
                                                        'gasPrice':gasprice, 'from': address, 'nonce':nonce})
        signed_tx =  sendfrom.signTransaction(unsigned_tx)
        txid = self.web3.eth.sendRawTransaction(signed_tx.rawTransaction)
        return txid.hex()

    def lowerToUpperStake(self, lower):
        chklower = web3.Web3.toChecksumAddress(lower)  
        ret = self.contract.caller().lowerToUpperStake(chklower)
        return ret
