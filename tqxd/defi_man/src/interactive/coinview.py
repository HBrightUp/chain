...
class CoinView(QDialog):
    db_account = None
    filter_result = []
    def __init__(self):
        super().__init__()
        from PyQt5.uic import loadUi
        ui_file = setting.g_setting.getPath() + setting.g_setting.getUi("coinview")
        loadUi(ui_file, self)
        #loadUi("../ui/coinview.ui", self)
        self.btn_dump_json.clicked.connect(self.onDumpJson)
        self.btn_dump_excel.clicked.connect(self.onDumpExcel)
        self.btn_search_sys.clicked.connect(self.onSearchSys)
    ...
    def showContractInfo(self,sql):
        model = QSqlTableModel()
        sql = "SELECT * FROM coin "
        contract_address = self.lineEdit_contract.text()
        if contract_address != "":
            sql = sql + " WHERE contract_address=%s"%(contract_address)
    ...
    def onSearchSys(self):
        dataformat.logger.info("on search system")
        self.pbar.show()
        ...
        url  = setting.g_setting.getRpcUrl()
        api = ETHAPI(url)
        sys = "%" + sys +"%"
        user = "%" + user +"%"
        coin = "%" + coin +"%"
        ...
        sql_data = "select address from account where  sys like '%s' and user like '%s' and coin like '%s'and logic like '%s'"%(sys,user,coin,logic)
        ...
        for index in range(account_count):
            ...
            model.setItem(index, 1, QtGui.QStandardItem(balance))
            list_add = []
            list_add.append(address)
            list_add.append(balance)
            for j in range(asset_count):
                contract_address = assets[j][1]
                amount = self.getBalance(address, contract_address)
    ...
    def onSearchAddress(self):
        ...
        if self.timer.isActive():
            ...
            self.timer.start(100, self)
        #coin = self.lineEdit_coin_outer.text();
        ...
        self.filter_result.append(list_tmp)
        list_add = []
        model.setItem(1, 0, QtGui.QStandardItem(address))
        list_add.append(address)
        balance = api.getBalance(address)
        model.setItem(1, 0, QtGui.QStandardItem(str(balance)))
        list_add.append(balance)
        for i in range(asset_count):
            contract_address = assets[i][1]
            amount = self.getBalance(address, contract_address)
            model.setItem(1, j+2, QtGui.QStandardItem(str(amount)))
            list_add.append(amount)
            self.filter_result.append(list_add)
            self.step = step*i
    ...
    def onSearch(self):
        ...
        for j in range(asset_count):
            coin_name = assets[j][0]
            model.setHorizontalHeaderItem(j+1,QtGui.QStandardItem(coin_name))
            list_tmp.append(coin_name)
        ...
        for index in range(account_count):
            ...
            for j in range(asset_count):
                ...
                amount = self.getBalance(address, contract_address)
                list_add.append(amount)
                model.setItem(index, j+1, QtGui.QStandardItem(str(amount)))
            self.filter_result.append(list_add)
            self.step = step*index
            self.pbar.setValue(step*index)
        self.tableView.horizontalHeader().setSectionResizeMode(QHeaderView.ResizeToContents)
